...
for i, model in enumerate(models):
	...
	for j in range(1, len(phasebins)):
        ...
        if i ==0: plt.errorbar(bin_average, (bindata-1.)*1e3*dilution, yerr = binsigma*1e3, linestyle='none', marker = '.', color = 'k', ecolor = 'k', zorder = 10, alpha = 0.8)
#	plt.plot(bin_average, (bindata-1.)*1e3*dilution,  linestyle = 'none', marker ='.', color = colors[i], zorder = 10, alpha = 0.8)
        ...
        plt.plot(phase, (bestfit-1.)*1e3*dilution, color = colors[i], label = labels[i], alpha = 0.7, linestyle = linestyle[i])
...
#labels = [labels[2]]#, labels[1], labels[0], "GCM"]
...
plt.savefig('model_comparison1234.pdf')
