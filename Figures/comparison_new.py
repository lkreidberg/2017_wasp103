...
path = "BD_spectra/"
#files = ["For_Laura1320+0409 (L3:) SED.txt", "0024-0158 (M9.5) SED.txt", "For_Laura0003-2822 (M7.5) SED.txt"]
files = ["For_Laura1320+0409 (L3:) SED.txt", "0428-2253 (L0.5) SED.txt", "For_Laura0003-2822 (M7.5) SED.txt"]
#names = ["1320+0409", "0428-2253", "0003-2822"]
names = ["1320+0409", "0024-0158", "0003-2822"]

print "PUT IN CORRECT DISTANCE AND RADIUS"

#Ts = [1875., 2385., 2889.]
Ts = [1875., 2429., 2889.]
#distance = np.array([30.96, 11.55, 38.91])			#parsecs
distance = np.array([30.96, 25.99, 38.91])			#parsecs
#radius = np.array([1.01, 1.1, 1.32])				#jupiter radii
radius = np.array([1.01, 1.09, 1.32])				#jupiter radii
unit_conversion = (distance/radius)**2*1.863e17		#(r/d)^2 in dimensionless units *10 to get in W/m^2/micron from ergs/A/cm^2/s
...
for i, f in enumerate(files):
    ...
    if i != 1: plt.plot(d[:,0], d[:,1]*unit_conversion[i]/scale)
    else: plt.plot(d[:,0], d[:,1]*unit_conversion[i]/scale)
