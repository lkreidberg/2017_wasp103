...
print "discrepancy with best fit blackbody (to WFC3 data only):"
Tbest = 3013
star = np.genfromtxt("wasp103_sed_fluxes.out")
star_bb = np.interp(x, star[:,0], star[:,1])*1.e24/(x*np.pi*4.)
model = blackbody(x*1.0e-6, Tbest)/star_bb*rprs**2
print model
print (y - model)/e
