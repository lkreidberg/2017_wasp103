...
for f in files:
        d = np.load(f)
        print f[32+offset:36+offset], np.median(d[:,ind]), np.median(d[:,ind]) - quantile(d[:,ind], 0.16), quantile(d[:,ind], 0.84) - np.median(d[:,ind]), np.median(d[:,ind2]), np.median(d[:,ind2]) - quantile(d[:,ind2], 0.16), quantile(d[:,ind2], 0.84) - np.median(d[:,ind2]), np.median(d[:,ind3]) - quantile(d[:,ind3], 0.16), quantile(d[:,ind3], 0.84) - np.median(d[:,ind3])
