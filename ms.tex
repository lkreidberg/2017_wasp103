...
Planets are round, rotating, and irradiated on one hemisphere at a time -- all of which contribute to rich spatial structure in their climate and atmospheric composition.  Short period, tidally locked planets are an extreme example, with hot, continuously illuminated daysides. This asymmetry is expected to produce large gradients in temperature, chemistry, and cloud coverage with longitude \citep{showman09, kataria16, parmentier16}.  FIXME we want to understand this, heat circulation and atmospheric dynamics, and also to put transmission and emission spectra in context. Extrasolar planets are so distant that they are not spatially resolved, so the only way to reveal this variation is to measure a phase curve. Phase curve observations consist of continuous monitoring of the planet-to-star flux ratio over a complete orbital revolution of the planet, which exposes different longitudes at each orbital phase.  

The first phase curve was observed with \Spitzer\ for the hot Jupiter HD 189733b by \citep{knutson07}, followed by additional \Spitzer\ observations for about a dozen more systems \citep{parmentier17}.  These observations revealed large day-night temperature contrasts (in excess of FIXME K), and eastward shifted peak brightness due to heat circulation.  These measurements were complemented by optical phase curves from \Kepler\, which are sensitive to reflected light \citep[e.g.]{esteves15}. FIXME clouds. \cite{stevenson14} observed the first \emph{spectroscopic} phase curve for WASP-43b with \emph{Hubble}/Wide Field Camera 3 (\HST/WFC3) in the near-infrared. These observations had fairly good agreement with 3D GCM predictions, eastward-shifted peak brightness and more efficient heat redistribution for cooler planets. 

In this paper we present spectroscopic phase curve observations of the hot Jupiter WASP-103b. This planet is an ideal target for thermal phase curve observations, with an orbital period of 22 hours and an equilibrium temperature of 2500 K \citep{gillon14}. The dayside thermal emission of WASP-103b was detected by \cite{cartier17}

	spin-orbit alignment: 2016ApJ...823...29A

Thermal emission spectra of other super-hot Jupiters

The atmosphere of WASP-103b has been characterized with transmission and emission spectroscopy \citep{southworth16, cartier17, lendl17}. The thermal emission spectrum is featureless 

%, a hot Jupiter with a mass of X, radius of Y, period of Z (cite discovery paper). These properties are similar to WASP-43b, except that WASP-103b has a much (FIXME vs FIXME K) thanks to its hot F?? star host. By comparing the phase curves of these two planets, we can test the effect of irradiation on the climate of the planet.
