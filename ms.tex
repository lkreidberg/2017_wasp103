...
We note that an important caveat for our results is that the best fit model is not a perfect fit to the data (with $\chi_\nu = 1.77$), so the uncertainties produced by the MCMC may be underestimated. Furthermore, the inferred C/O and metallicity are highly sensitive to the large planet-to-star flux at \Spitzer $4.5\mu$m, which is the worst fit data point. (With this point removed, the fit quality is significantly improved: $\chi_\nu = 1.17$).  The dominant absorbing species over the $4.5\,\mu$m bandpass is CO, so to fit this point the model tends to favor super-solar metallicities and C/O. Given that the result is driven by a single photometric band and there are no resolved molecular features to constrain the absorbers that are present or the temperature-pressure profile, we caution against over-interpreting these results until wider spectral coverage is available.
