...
\begin{deluxetable}{llllllll}
	...
	\tablehead{
	\colhead{Data} & \colhead{Model} & \colhead{$T_\mathrm{min}$} & \colhead{$T_\mathrm{max}$} & \colhead{$\overline{T}_\mathrm{night}$} & \colhead{$\overline{T}_\mathrm{day}$} &\colhead{$\Delta_\mathrm{AIC}$} & \colhead{$\Delta_\mathrm{BIC}$}}
		\startdata
...
Each phase curve observation includes a transit of WASP-103b. To measure the wavelength-dependent transit depths (the transmission spectrum), we select a subset of each phase curve. The subset includes the transit and additional baseline on either side, such that the total light curve has twice the duration of the transit. Over this short duration, there is negligible curvature in the light curve due to the planet's thermal phase variation. We fit the data with a transit model, which has free parameters for the planet-to-star radius and a linear limb darkening parameter. We fit for the instrument systematics using the same parameters as the phase curve fits, except that we modeled the visit-long systematic as a linear trend in time (which is sufficient for the shorter duration). The advantage of fitting the transits separately from the full phase curves is that the resultant transit depths are not dependent on how the phase variation is modeled. We do not model the transit in the \Spitzer\,$3.6\mu$m channel, since it is challenging to robustly estimate the contribution of correlated noise in the subset of data we model here.  FIXME

We show the measured transmission spectrum in Figure,\ref{fig:tspec}.  The spectrum is biased by flux contamination from the planet's nightside; to correct for nightside emission, we estimate the planet-to-star flux ratio and subtract it from the transit depth. We calculate planet-to-star flux assuming a FIXME PHOENIX model for the star, a blackbody for the planet, and a planet-to-star radius ratio of FIXME. We consider two plausible nightside temperatures (1400 and 2000 K), which encompass the range of mean nightside temperatures calculated from the phase variation models (Table\,\ref{tab:FIXME}). We also show the uncorrected transit depths. For the uncorrected data, there is an offset between the \HST\ and \Spitzer\ data of about 10 atmospheric scale heights, much larger than expected from theoretical models. FIXME show GCM in this figure. Water abundance?

\begin{figure}
\includegraphics[width = 0.5\textwidth]{Figures/w103b_transmission_spectrum.pdf}
\caption{The transmission spectrum of WASP-103b (points) for a range of assumptions about the planet's nightside temperature. The gray points correspond to no correction for the nightside flux. The blue and red points assume the average nightside photospheric temperature is 1400 and 2000 K, respectively. The atmospheric scale height assumes FIXME.}
\label{fig:tspec}
\end{figure}
...
\begin{deluxetable}{LLL}
	...
	\tablehead{
	\colhead{Orbital Phase} & \colhead{$T_\mathrm{b}$\tablenotemark{1}} & \colhead{$\chi^2_\nu$}}
		\startdata
		0.06 - 0.15 & 1917 \pm 43 & 0.7 \\
		0.15 - 0.25 & 2254 \pm 25 & 0.8 \\
		0.25 - 0.35 & 2650 \pm 17 & 1.4 \\
		0.35 - 0.44 & 2905 \pm 11 & 1.1 \\
		0.44 - 0.56 & 3011 \pm 8 & 2.4 \\
		0.56 - 0.65 & 2884 \pm 11 & 1.7 \\
		0.65 - 0.75 & 2634 \pm 16 & 1.1 \\
		0.75 - 0.85 & 2312 \pm 23 & 1.2 \\
		0.85 - 0.94 & 2025 \pm 34 & 0.5 \\
		\enddata
		\vspace{-0.8cm}
