...
\caption{Abundances of key molecular species for different viewing geometries, from the $\tau_\mathrm{drag3}$ GCM.}
...
\begin{itemize}
	\item{We fit the climate using the \texttt{spiderman} package \citep{louden17}}
	\item{The phase curves have large amplitude ($0.8 -0.9\times$ secondary eclipse depth)}
	\item{The phase-resolved spectra are consistent with a blackbody model, but there is an emission feature at \Spitzer\ wavelengths. Why is it isothermal? cite Beatty} 
	\item{We characterized the composition with a 1D grid-based retrieval. The atmospheric metallicity is super-solar () and > 1 at $3\,\sigma$ confidence. larger than  } 
	\item{We compared the phase curves to 3D GCM predictions}
	\item{We compared the spectra to brown dwarfs and directly imaged companions of similar temperature}
\end{itemize}
