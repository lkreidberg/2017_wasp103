...
To assess how well the GCM predictions reproduce the data, we calculate the amplitude and hot spot offset for all the models (listed in Table\,\ref{tab:amps_offs}). The models with drag fit better, with phase variation amplitude near unity and a hot spot offset near zero for all three band-passes. The nominal model and metal-enhanced composition ([Fe/H] = 0.5) both have a phase curve offset that is too large (
