...
%\section{Analysis}
%In this section we describe the interpretation of the data using simple blackbody models, a 1-D grid-based retrieval, and 3-D general circulation models (GCMs).
...
A blackbody is the simplest model for the planet's thermal emission and provide a useful first evaluation of the atmospheric properties. The best fit blackbodies are shown in Figures \ref{fig:spectra} and \ref{fig:dayside}. This model performs well for some orbital phases, with $\chi^2_\nu$ ranging from 0.9 to 1.1 for phases $\phi = 0.1, 0.7, \mathrm{and} 0.8$. At other phases the blackbody model is a poorer fit, particularly to the \Spitzer\ data. For most of the eastern hemisphere to just past the substellar point (phases $0.2 - 0.6$), the \Spitzer\ eclipse depths are larger than predicted for the best fit blackbody, suggesting the presence of an emission feature. On the nightside (phase $0.9$), the \Spitzer\ $4.5\,\mu$m eclipse depth is \emph{smaller} than predicted, suggesting an absorption feature. These differences hint at changing thermal structure with longitude in the atmosphere.

We also fit blackbody models to the WFC3 data only, and found that they provide an excellent fit at all phases except the dayside. The $\chi^2_\nu$ values range from 0.5 to 1.4, with a median of 0.9. On the other hand, the dayside spectrum is inconsistent with a blackbody ($chi^2_\nu$ = 5.0). (FIXME: what features are there?).  The dayside spectrum has higher signal-to-noise than the other phases thanks to the two secondary eclipse observations from \citep{cartier17}, so it is not surprising that a more complex model is required.  In Table\,\ref{tab:depths} we report the best fit temperatures for the WFC3 data blackbody fits.
...
\subsubsection{Other Orbital Phases}
We also fit the nightside spectrum with the grid-based retrieval. 
...
\subsection{Comparison with GCMs}
