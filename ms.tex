...
\label{sec:bbfits}
A blackbody is the simplest model for the planet's thermal emission and provides a useful first evaluation of the atmospheric properties. To calculate the best fit blackbody model, we assumed a planet-to-star radius ratio of 0.1146 and used a NextGen stellar spectrum interpolated to an effective temperature of 6110 K \citep{allard12}.  The best-fit blackbodies are shown in Figures \ref{fig:spectra} and \ref{fig:dayside}. The model is consistent with the data at the $3\,\sigma$ level for phases 0.1 - 0.7. At later phases, the \Spitzer\ data have significantly lower brightness temperatures.  

We also fit indepedent blackbody models to the \HST/WFC3 data and each \Spitzer\ channel. The resulting brightness temperatures are listed in Table\,\ref{table:teffs}. The WFC3 data are fit well by a blackbody model at all orbital phases except phase 0.5 (consistent at better than $1.5\,\sigma$). The dayside has higher signal-to-noise than the other orbital phases, thanks to the two secondary eclipse observations from \cite{cartier17}, and the more sophisticated grid-based retrieval (described in the next section) provides a better fit to the dayside.
FIXME talk about significance of features.
