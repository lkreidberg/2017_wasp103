...
We fit a two-component model to the full-orbit phase curves. One component models the planet signal (the thermal phase variation and transit), and the other component is a systematics model for changes in the instrument sensitivity. For each light curve, we fit the physical and systematic component simultaneously. 

\subsection{Thermal Phase Variation Models}
We explored several different models for the planet's thermal phase variation. The simplest model was a sine curve, which has been used for many past analyses of exoplanet phase curves \citep[e.g.]{FIXME}. 

We also fit climate maps using the open-source Python package \texttt{SPIDERMAN}. This package allows users to input a climate map (temperature or brightness as a function of latitude and longitude), and outputs a the thermal phase curve. The code is optimized to calculate   
