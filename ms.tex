...
\caption{The transmission spectrum of WASP-103b, corrected for planet nightside emission at 1700 K (blue points) and uncorrected (red points). The dark gray line is the model transmission spectrum from the $\tau_\mathrm{drag4}$ GCM. Atmospheric scale height $H$ is shown on the right y-axis, where $H = 5.5\times10^6$ m (assuming a mean molecular weight of 2.3 atomic mass units, surface gravity $g = 15.9$ m/s$^2$, and a temperature $T = 2410$ K).}
\label{fig:tspec}
\end{figure}
...
\label{sec:bbfits}
A blackbody is the simplest model for the planet's thermal emission and provides a useful first evaluation of the atmospheric properties. To calculate the best fit blackbody model, we assumed a planet-to-star radius ratio of 0.1146 and used a NextGen stellar spectrum interpolated to an effective temperature of 6110 K \citep{allard12}.  The best-fit blackbodies are shown in Figures \ref{fig:spectra} and \ref{fig:dayside}. The model fits the data fairly well: it is consistent with the data at the $3\,\sigma$ level for 70\% of the orbital phases. At phase 0.5, the \Spitzer\ data imply higher brightness temperatures than the WFC3 data, whereas at phases $0.8 - 0.9$ the \Spitzer\ data have lower brightness temperatures.  These differences suggest the presence of an emission feature on the dayside at \Spitzer\ wavelengths, which transitions to an absorption feature on the nightside, perhaps indicating changes in thermal structure with longitude in the atmosphere.

We also fit indepedent blackbody models to the \HST/WFC3 data and each \Spitzer\ channel. The resulting brightness temperatures are listed in Table\,\ref{table:teffs}. The WFC3 data agree well by a blackbody model at all orbital phases except phase 0.5 (consistent at better than $1.5\,\sigma$). The dayside has higher signal-to-noise than the other orbital phases, thanks to the two secondary eclipse observations from \cite{cartier17}, and the more sophisticated grid-based retrieval (described in the next section) provides a better fit to the dayside.
...
\begin{deluxetable}{LLLLL}
	...
	\colhead{\,} & \colhead{WFC3} & \colhead{Ch. 1} & \colhead{Ch. 2} & \colhead{(9 dof)}}
		\startdata
		0.06 - 0.15 & 1883 \pm 41 & 1523 \pm 158 & 1589 \pm 98 & 0.7 \\
		0.15 - 0.25 & 2208 \pm 24 & 2612 \pm 104 & 2299 \pm 78 & 0.9 \\
		0.25 - 0.35 & 2587 \pm 16 & 2926 \pm 95 & 2592 \pm 67 & 1.5 \\
		0.35 - 0.44 & 2831 \pm 11 & 2834 \pm 93 & 2976 \pm 61 & 1.5 \\
		0.44 - 0.56 & 2933 \pm 8 & 2995 \pm 141 & 3154 \pm 48 & 2.8 \\
		0.56 - 0.65 & 2811 \pm 11 & 2631 \pm 96 & 2891 \pm 63 & 2.0 \\
		0.65 - 0.75 & 2572 \pm 15 & 2708 \pm 101 & 2421 \pm 78 & 1.3 \\
		0.75 - 0.85 & 2263 \pm 22 & 1952 \pm 117 & 1939 \pm 83 & 1.3 \\
		0.85 - 0.94 & 1987 \pm 34 & 1594 \pm 150 & 1288 \pm 118 & 0.6 \\
		\enddata
...
We infer a heat redistribution $f = 2.49^{+0.14}_{-0.15}$. An $f$ parameter of unity represents isotropic heat distribution, whereas $f = 2$ corresponds to dayside emission only. We estimate $f > 2$, indicating a thermal inversion and likely inefficient transport of heat to the nightside. 
%Poor heat transport is observed in other very hot planets as well, due to damping of heat-propagating waves by fast radiative cooling or frictional drag in a partially ionized atmosphere \citep{komacek17}.  
The heat redistribution is strongly correlated with atmospheric metallicity because increasing metallicity  shifts the T-P profile to lower pressures, resulting in hotter temperatures at a given pressure level (equivalent to less efficient heat redistribution). 
...
To check that fast drag timescales of order $10^4$\,s are physically realistic, we computed the free electron abundance due to ionized metals for the $\tau_\mathrm{drag4}$ GCM. Using the order-of-magnitude expression from \cite{perna10} (Equation 12), we estimate a drag timescale at the substellar point of $\tau_\mathrm{drag}  = 2\times10^4/B^2$ s, where $B$ is the magnetic field in Gauss. We assume a temperature and pressure of 3359 K and 0.11 bar, and that the magnetic field is perpendicular to the flow. To reach a drag timescale of $10^4$\,s, a magnetic field stronger than $\sim1$ Gauss is required. This value is comparable to Jupiter's magnetic field strength of 5-10 Gauss \citep{bagenal04}.

We also compared the TP profiles from the GCM to cloud condensation curves and the best fit radiative-convective equilibrium models from the 1D retrieval (Figure\,\ref{fig:TP}). The 3D and 1D models agree fairly well in the photosphere (at a pressure of $10^{-1} -10^{-3}$ bars). FIXME: why doesn't the deep atmosphere agree? The photosphere is at higher altitudes than the intersection points for the condensation curves and the TP profiles, suggesting that the observable atmosphere is cloud-free. 

\begin{figure}
\includegraphics[width = 0.5\textwidth]{Figures/WASP-103b-NoClouds-Drag3-nit-LR.pdf}
\caption{TP profiles from the GCM (thin colored lines) and 1D retrievals (thick lines), compared to condensation curves of potential cloud species. The blue, green, and red lines correspond to TP profiles randomly drawn from the FIXME GCM at orbital phases FIXME. The thick red and blue lines are the best fit 1D TP profiles for the dayside ($\phi = 0.5$) and nightside ($\phi = 0.1$), respectively. The dotted lines correspond to regions of constant H$_2$O abundance, with numbers indicating the log10(H$_2$O volume mixing ratio).}
\label{fig:TP}
\end{figure}
...
\begin{deluxetable*}{llCCCC}
	...
	\colhead{\,} & \colhead{Object} & \colhead{$T_\mathrm{eff}$ (K)} & \colhead{log\,$g$ (cgs)} & \colhead{H$_2$O A$_1$} & \colhead{H$_2$O A$_2$}}
		...
		\, & W103b quadrature & 2400 \pm 20 & 3.2 \pm 0.04 & -0.14 \pm 7.8\mathrm{e}-02 & -0.01 \pm 6.7\mathrm{e}-02 \\
		\, & W103b dayside & 2930 \pm 10 & 3.2 \pm 0.04 & 0.04 \pm 1.4\mathrm{e}-02 & 0.15 \pm 1.2\mathrm{e}-02 \\
...
\item{We calculated phase-resolved spectra in ten orbital phase bins. The \HST/WFC3 spectra are consistent with  blackbody emission from the planet at all orbital phases. The best fit brightness temperatures ranges from $1880\pm40$ K (phase $\phi = 0.1$) to $2930\pm10$ K on the dayside. We attribute the absence of water features at WFC3 wavelengths to (1) H$_2$O dissociation on the dayside and (2) additional near-IR opacity from H-, TiO/VO and FeH.}
\item{The \Spitzer\ data are \emph{not} consistent with the best fit blackbody to the WFC3 data: they have a higher brightness temperature at phases $\phi = 0.2 - 0.5$, which transitions to a lower brightness temperature on the nightside ($\phi = -0.2 - 0.1$). These characteristics are likely due to CO features in the infrared and changing thermal structure from the dayside to the nightside.}
